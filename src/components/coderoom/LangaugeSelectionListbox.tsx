...
export default function LanguageSelectionListbox({ setCurrentLanguage }: { setCurrentLanguage: Dispatch<SetStateAction<string>> }) {
                        ...
                        <Listbox.Options className="absolute mt-1 max-h-60 w-max overflow-auto rounded-md bg-light-100 dark:bg-dark-100 py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                            {LANGUAGES.map((language, index) => (
                                ...
                                >
                                    {({ selected, active }) => (
                                        <>
                                            <span className={`block truncate ${selected ? "font-medium" : "font-normal"}`}>{language.language}</span>
                                            {selected ? (
                                                <span
