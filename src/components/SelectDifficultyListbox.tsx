    ...
    return (
        <div className="my-10">
            <Listbox value={selectedDifficulty.sort((a, b) => (a.id > b.id ? 1 : a.id < b.id ? -1 : 0))} onChange={setSelectedDifficulty} multiple>
                <Listbox.Button className="relative w-60 h-max cursor-pointer rounded-lg bg-white dark:bg-dark-200 py-2 pl-3 pr-10 text-left focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                    <span className="truncate flex justify-center items-center gap-4">
                ...
                <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
                    <Listbox.Options className="fixed overflow-auto rounded-md bg-light-100 dark:bg-dark-200 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                        {DIFFICULTY.map((d) => (
                            <Listbox.Option
                                key={d.id}
                                value={d}
                                className={({ active }) =>
                                    `relative cursor-pointer select-none py-2 pl-10 w-60 ${
                                        active ? "bg-green-200 text-light-100 dark:text-dark-200" : "text-dark-200 dark:text-light-100"
                                    }`
                                }
                            >
                                {({ selected, active }) => (
                                        ...
                                        {selected ? (
                                            <span
                                                className={`absolute inset-y-0 left-0 flex items-center pl-3 
                                                    ${active ? "text-light-100 dark:text-dark-200" : "text-green-200"}
                                                    `}
                                            >
