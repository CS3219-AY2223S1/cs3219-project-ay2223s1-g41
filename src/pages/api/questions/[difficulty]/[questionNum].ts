...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (difficulty == 'easy') {
    ...
    .collection(process.env.MONGO_QN_EASY!);
  } else if (difficulty == 'medium') {
    collection = client
    .db(process.env.MONGO_DB)
    .collection(process.env.MONGO_QN_MEDIUM!);
  } else {
    collection = client
    .db(process.env.MONGO_DB)
    .collection(process.env.MONGO_QN_HARD!);
  }
  ...
  res
    .status(200)
    .json({ message: "Fetched a random easy question.", question: question});
}
